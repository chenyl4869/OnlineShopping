<template>
  <div>
              ...
              <li v-for = "(item,index) in cartList">
                <div class="cart-tab-1">
                  ...
                  <div class="cart-item-pic">
                    <img v-bind:src="`/static/${item.productImage}`" v-bind:alt="`${item.productName}`"></a>
                  </div>
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click = "deleteItem(item.productId)">
    ...
    <model v-bind:mdShow="mdShow" v-on:close="closeModel">
      <p slot = "message">Are you sure you want to delete all of your items {{}}</p>
      <div slot = "btnGroup">
    ...
    export default{
        ...
        computed:{
          ...
          totalPrice(){
            ...
            this.cartList.forEach((item)=>{
              if(item.checked == '1'){
                res += parseFloat(item.salePrice) * parseFloat(item.productNum);
              }
            });
            return res;
          }
        ...
        methods:{
          ...
          delCart(){
            axios.post("/users/cart/del", {productId: this.productId}).then((result)=>{
              let res = result.data;
              if(res.status == "0"){
          ...
          editCart(flag, item){
            ...
            axios.post("/users/cartEdit",{productId:item.productId, productNum:item.productNum, checked:item.checked}).then((response)=>{
          ...
          toggleCheckAll(){
            ...
            axios.post("users/editCheckAll",{checked: flag?1:0}).then((response)=>{
              let res = response.data;
              //console.log(res.status);
            });
          },
